...
class Deb::S3::Package
  ...
  def url_filename
    @url_filename || "pool/#{self.name.split('')[0]}/#{self.name[0..1]}/#{File.basename(self.filename)}"
  end

  def url_filename_encoded
    @url_filename || "pool/#{self.name.split('')[0]}/#{self.name[0..1]}/#{s3_escape(File.basename(self.filename))}"
  end
